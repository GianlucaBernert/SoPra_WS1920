...
CREATE TABLE IF NOT EXISTS `popcorns`.`cinema` (
  ...
  `cinemachainFK` INT(11) NULL,
  PRIMARY KEY (`bo_id`),
  INDEX `fk_cinema_businessownership1_idx` (`bo_id` ASC) VISIBLE,
  INDEX `fk_cinema_cinemachain1_idx` (`cinemachainFK` ASC) VISIBLE,
  CONSTRAINT `fk_cinema_businessownership1`
    FOREIGN KEY (`bo_id`)
    REFERENCES `popcorns`.`businessownership` (`bo_id`),
  CONSTRAINT `fk_cinema_cinemachain1`
    FOREIGN KEY (`cinemachainFK`)
    REFERENCES `popcorns`.`cinemachain` (`bo_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
